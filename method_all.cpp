...
// void decode(const INode* node, string s)
// {
//     if (const LeafNode* lf = dynamic_cast<const LeafNode*>(node))
//     {
//         outCodes[lf->c] = prefix;
...
u_int32_t decode_file(const INode* root, string s)
{
    string ans = "";
    u_int32_t ansint=0;
...
class COLESS{
	...
	void get_next_value(InputFile infile){
		return;
	}
	...
	void method1_pass1(){
		...
		load_huffman_table();//int M -> variable string   //writehuffman[int]
		...
		set<int> local_col_classes_uniq; //get the bool HuffCodeMap[M-1]
		bool use_local_hash = false;
		uint64_t skip=0
		int case_run = 0;
		int case_lm = 0;
		int case_dlc = 0;
		...
		for (uint64_t i=0; i < num_kmers; i+=1){ 
			...
			if(spss_boundary[i]=='0'){ // non-start
				...
				sum_length_huff += HuffCodeMap[col_class_id].length();
			...
			if(spss_boundary[(i+1)%num_kmers]=='1'){	// end k-mer of simplitig
				...
				use_local_hash1 = ( (ll - lm ) * case_lm_1 + lm (1+l) < 0 ) ;  //ll*case_lm + (lm + l*lm) ::: lm * case_lm 
				use_local_hash_huff = ( (ll*case_lm - sum_length_huff + lm + sum_length_huff_uniq) < 0);
				use_local_hash_huff1 = ( ll*case_lm - sum_length_huff1 + lm + sum_length_huff_uniq1 < 0 );

				logfile_main.fs<<use_local_hash<<" "<<use_local_hash1<<" "<<use_local_hash_huff<<" "<<use_local_hash_huff1<<" "<<num_kmer_in_simplitig<<endl;
				
				//re-init for new simplitig
				vector<int>().swap(local_col_classes_uniq);//local_col_classes_uniq.clear();
...
int main (int argc, char* argv[]){
    ...
    for (auto i = args.begin(); i != args.end(); ++i) {
        if (*i == "-h" || *i == "--help") {
            cout << "Syntax: tool -i <DE-DUP-bitmatrix> -d <dup-bitmatrix> -c <num-colors> -m <M> -k <num-kmers> -s <spss-bound>" << endl;
            return 0;
        } else if (*i == "-i") {
	...
	COLESS coless(cmp,  num_kmers, M, C, string dedup_bitmatrix_fname, string dup_bitmatrix_fname, string spss_boundary_fname);
	coless.pass1();
